      ...
      try {
        ...
        if (obj.result == 0) {
          ...
          for (list of obj.items) {
            (productOFFName = list.productOFFName), (offerType = list.offerType);
      ...
      try {
        ...
        if (obj.serviceResultCode == "0") {
          ...
          for (bills of obj.items[0].items) {
            (chargeName = bills.chargetypeName), (charge = bills.charge);
  ...
  return new Promise((resolve, reject) => {
    $.get(requests.info, (err, resp, data) => {
      try {
        let obj = JSON.parse(data);
        if (obj.result == 10000) {
          (mobileShort = obj.mobileShort), (city = obj.city);
    ...
    $.post(requests.balance, (err, resp, data) => {
      try {
        ...
        if (obj.result == 0) {
          ...
          $.log(
            "您的余额总计" + totalBalance + "通用余额为" + general + (special ? "专用余额为" + special : "") + "\n"
          );
    ...
    $.post(url, (err, resp, data) => {
      try {
        ...
        if (obj.resCode == "00000") {
          signres =
            obj.data.status == 1
              ? "签到成功，已签到" + obj.data.signNum + "天"
              : "今日已签到，总计签到" + obj.data.signNum + "天";
          $.log("\n签到兑豪礼: " + signres);
