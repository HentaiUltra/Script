...
function HTTP(e = { baseURL: "" }) {
  function t(t, u) {
    ...
    const c = e.baseURL;
    c && !a.test(u.url || "") && (u.url = c ? c + u.url : u.url),
      u.body &&
        u.headers &&
    ...
    if ((l.onRequest(t, u), s)) f = $task.fetch({ method: t, ...u });
      ...
      f = new Promise((e, s) => {
        ...
        n[t.toLowerCase()](u, (t, n, i) => {
          t ? s(t) : e({ statusCode: n.status || n.statusCode, headers: n.headers, body: i });
        });
      });
    else if (o) {
    ...
    const p = h
      ? new Promise((e, s) => {
          d = setTimeout(() => (l.onTimeout(), s(`${t} URL: ${u.url} exceeds the timeout ${h} ms`)), h);
        })
      : null;
    return (p ? Promise.race([p, f]).then((e) => (clearTimeout(d), e)) : f).then((e) => l.onResponse(e));
  }
  const { isQX: s, isLoon: n, isSurge: i, isScriptable: o, isNode: r } = ENV(),
    u = ["GET", "POST", "PUT", "DELETE", "HEAD", "OPTIONS", "PATCH"],
  ...
  return new (class {
    ...
    initCache() {
      if (
        ...
        (n || i) && (this.cache = JSON.parse($persistentStore.read(this.name) || "{}")),
        o)
      ) {
        let e = "root.json";
    ...
    persistCache() {
      ...
      s && $prefs.setValueForKey(e, this.name),
        (n || i) && $persistentStore.write(e, this.name),
        o &&
          (this.node.fs.writeFileSync(`${this.name}.json`, e, { flag: "w" }, (e) => console.log(e)),
          this.node.fs.writeFileSync("root.json", JSON.stringify(this.root, null, 2), { flag: "w" }, (e) =>
            console.log(e)
          ));
    }
    write(e, t) {
    ...
    read(e) {
      return (
        ...
        -1 === e.indexOf("#")
          ? this.cache[e]
          : ((e = e.substr(1)),
            i || n ? $persistentStore.read(e) : s ? $prefs.valueForKey(e) : o ? this.root[e] : void 0)
      );
    }
    delete(e) {
    ...
    notify(e, t = "", a = "", c = {}) {
      ...
      if ((s && $notify(e, t, a, c), i && $notification.post(e, t, a + `${l ? "\n多媒体:" + l : ""}`, { url: h }), n)) {
        ...
        h && (s.openUrl = h),
          l && (s.mediaUrl = l),
          "{}" === JSON.stringify(s) ? $notification.post(e, t, a) : $notification.post(e, t, a, s);
      }
      if (o || u) {
        const s = a + (h ? `\n点击跳转: ${h}` : "") + (l ? `\n多媒体: ${l}` : "");
        if (r) {
