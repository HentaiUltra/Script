  ...
  if (check("watch")) {
    ...
    return await $.http
      .post({
        ...
        body: `aid=${aid}&cid=${cid}&bvid=${bvid}&mid=${config.user.mid}&csrf=${config.cookie.bili_jct
          }&played_time=1&real_played_time=1&realtime=1&start_ts=${Date.parse(new Date()) / 1000
...
!(async () => {
  ...
  if (config.cookie && (await me())) {
    ...
    let title = `${name} 每日任务 ${config.user.num}/${config.watch.num}/${config.share.num
      }${config.done ? "" : "未完成"}`;
    console.log(`#### ${title}`);
    ...
    return {
      ...
      content:
        ...
        `任务:登录(观看)${check("watch") ? "" : "+10exp"} 分享${check("share") ? "" : "+5exp"
        }\n` +
        `经验:${config.user.level_info.current_exp}/${config.user.level_info.next_exp}/28800\n` +
        `等级:${config.user.level_info.current_level} ==> ${config.user.next_day
